...
const renderers = {
        ...
        <div class="stage-section-grid">
          ${slide.sections
            .map(
              ...
              <article>
                <h3>${escapeHtml(section.title)}</h3>
                ${renderTable(section.columns, section.rows, "compact-matrix")}
              </article>`
            )
            .join("")}
        </div>` : ""}
  ...
  processDetail(slide) {
    const rowMidpoint = Math.ceil(slide.rows.length / 2);
    const rowGroups = [slide.rows.slice(0, rowMidpoint), slide.rows.slice(rowMidpoint)];
    return `
      <section class="slide process-detail-slide" data-title="${escapeHtml(slide.title)}">
        <header class="slide-header">
          <p class="eyebrow">${escapeHtml(slide.eyebrow)}</p>
          <h2>${escapeHtml(slide.title)}</h2>
        </header>
        <div class="note-panel"><p>${escapeHtml(slide.story)}</p></div>
        <div class="process-table-split">
          ${rowGroups.map((rows) => renderTable(slide.columns, rows, "compact-matrix")).join("")}
        </div>
        <div class="process-summary-grid">
          ${slide.lanes
            .map(
              ...
              <article>
                <strong>${escapeHtml(role)}</strong>
                <span>${steps.map((step) => escapeHtml(step)).join(" → ")}</span>
              </article>`
            )
            .join("")}
        </div>
